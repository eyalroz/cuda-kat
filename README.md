...
| [`streams/*.cuh`](https://github.com/eyalroz/cuda-kat/blob/master/src/kat/on_device/streams/)          | `stringstream ss; ss << "easy as " << 123 << '!'; use_a_string(ss.c_str());`<br>`printfing_ostream cout; cout << "This text and number (" << 123.45 << ")\nget printfed on endl/flush" << flush;`<br><br>Stream functionality using dynamic memory allocation and/or CUDA's device-side `printf()`. Also supports automatic self-identification of the printer thread. |
